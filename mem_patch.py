...
def create_patch(infile, outfile):
    ...
    with open(infile, 'r+b') as bm2dx:
        with open(outfile, 'w', newline='\r\n') as patches:
            ...
            if game_version >= 30:
                # X rev
                mm.seek(mm.find(b'\x44\x24\x40\x85\xC0\x0F\x84\xF8\x00\x00\x00', 0x650000) + 5)
                print(f'bm2dx.dll {hex(pe.get_rva_from_offset(mm.tell()))[2:].upper()} 90E948 {mm.read(3).hex().upper()}', file=patches)
                mm.seek(mm.find(b'\xC6\x40\x05\x41', mm.tell()) + 3)
                print(f'bm2dx.dll {hex(pe.get_rva_from_offset(mm.tell()))[2:].upper()} 58 {mm.read(1).hex().upper()}', file=patches)
                ...
                print(f'bm2dx.dll {hex(pe.get_rva_from_offset(mm.tell()))[2:].upper()} EB {mm.read(1).hex().upper()}', file=patches)
            ...
            if game_version >= 30:
                ...
                mm.seek(mm.find(b'\x41\xB9\x04\x00\x00\x00\x4C\x8B\x44\x24\x70\xBA\x0D\x00\x00\x00\x48\x8B\x4C\x24\x50\xE8', 0x500000) + 2)
                while mm.read(5) != b'\x0F\xB6\xC0\x85\xC0':
                    mm.seek(mm.tell() - 6)
                mm.seek(mm.tell() - 10)
                print(f'bm2dx.dll {hex(pe.get_rva_from_offset(mm.tell()))[2:].upper()} B801000000 {mm.read(5).hex().upper()}', file=patches)
